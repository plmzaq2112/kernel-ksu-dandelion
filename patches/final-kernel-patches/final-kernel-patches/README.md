...
全部 7 个文件已与构建源码逐一 md5 校验一致（未做任何转换）。

| 文件 | 改动点 |
|---|---|
| `core/init.c` | late-init（`ksu_throne_tracker_init()` 之后）立刻 `track_throne(false)` 早抓一次 Manager；`MODULE_IMPORT_NS` 条件门控到 `>= 5.4`（4.19 无 symbol namespace，否则 4.19 编译报错） |
...
| `manager/throne_tracker.c` | 开机持久化：`ksu_throne_tracker_init` 调度 30s delayed work，`track_throne(false)` 扫描 `/data/app`（packages.list -> uid -> base.apk）找 Manager；失败每 30s 重试、最多 10 次，成功即自行停止。首次成功扫描后 **`ksu_load_allow_list()` 自动加载授权列表**（static bool 防重，修复"重启后授权全掉"——v3 Manager 不上报 post-fs-data 事件）。`is_manager_apk` 需**v2 证书签名**匹配（含 v3 签名的 APK 直接拒绝——`gkipatched.apk` 失败、`gkipatched_v2only.apk` 成功都因此） |
